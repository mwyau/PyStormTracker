...
from .io.json import read_json, write_json, infer_track_type
...
def generate_html(tracks: Tracks, outfile: str | Path, split: bool = False) -> None:
    ...
    if split:
        ...
        if "// TRACKS_DATA_PLACEHOLDER" in html_content:
            html_content = html_content.replace("// TRACKS_DATA_PLACEHOLDER", script_tag)
        else:
            html_content = html_content.replace('<script src="tracks_data.js"></script>', script_tag)
    else:
        injected_script = f"<script>\nwindow.TRACKS_DATA = {json_str};\n</script>"
        
        if "// TRACKS_DATA_PLACEHOLDER" in html_content:
            html_content = html_content.replace("// TRACKS_DATA_PLACEHOLDER", injected_script)
        else:
            html_content = html_content.replace('<script src="tracks_data.js"></script>', injected_script)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Convert PyStormTracker data between formats and generate interactive HTML visualizations. "
    ...
    parser.add_argument("-f", "--in-format", choices=["imilast", "json"], required=True, help="Input file format")
    parser.add_argument("-F", "--out-format", choices=["imilast", "hodges", "json", "html"], required=True, help="Output file format")
    parser.add_argument("--type", choices=["msl", "vo"], help="Override track type (msl or vo)")
    parser.add_argument("--split", action="store_true", help="For 'html' output, generate separate .html and .tracks.js files.")
