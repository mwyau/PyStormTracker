...
def infer_track_type(tracks: Tracks) -> str:
    ...
    if "msl" in tracks.vars:
        ...
        var_key = list(tracks.vars.keys())[0]
...
def write_json(tracks: Tracks, outfile: str | Path) -> None:
    ...
    def inject_nans(arr: NDArray[Any], insert_val: Any = np.nan) -> NDArray[Any]:
    ...
    metadata = {
        ...
        "max_duration": float(max(t["duration"] for t in tracks_meta)) if tracks_meta else 0.0,
        "max_displacement": float(max(t["displacement"] for t in tracks_meta)) if tracks_meta else 0.0,
    }
...
def read_json(infile: str | Path) -> Tracks:
    ...
    var_key = "msl" if track_type == "msl" else ("vo" if track_type == "vo" else "intensity")
