...
def test_refine_center_preserves_negative_longitudes() -> None:
    frame = np.array([[0.0, 0.5, 0.0], [0.6, 1.0, 0.4], [0.0, 0.5, 0.0]])
    lat = np.array([10.0, 11.0, 12.0])
    lon = np.array([-180.0, -179.0, -178.0])

    refined_lat, refined_lon, refined_value = refine_center(
...
def test_refine_center_keeps_projected_x_nonperiodic() -> None:
    frame = np.array([[0.0, 0.5, 0.0], [0.6, 1.0, 0.4], [0.0, 0.5, 0.0]])
    y = np.array([-100.0, 0.0, 100.0])
    x = np.array([-100.0, 0.0, 100.0])

    refined_y, refined_x, refined_value = refine_center(
...
def test_refine_center_does_not_wrap_projected_boundary() -> None:
    frame = np.ones((3, 3), dtype=np.float64)
    y = np.array([-100.0, 0.0, 100.0])
    x = np.array([-100.0, 0.0, 100.0])

    refined = refine_center(frame, 1, 0, y, x, periodic_x=False)
