...
def test_tracker_mpi_backend() -> None:
    ...
    with patch("pystormtracker.simple.concurrent.run_simple_mpi", return_value=Tracks()) as mock_run_mpi:
...
def test_tracker_dask_backend() -> None:
    ...
    with patch("pystormtracker.simple.concurrent.run_simple_dask", return_value=Tracks()) as mock_run_dask:
