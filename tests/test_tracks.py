...
def test_track_methods() -> None:
    ...
    # extend
    t2 = Tracks()
    track2 = t2.add_track([Center(t0 + np.timedelta64(12, "h"), 2, 2, 2)])
    track.extend(track2)
    assert len(track) == 3
...
def test_tracks_sort() -> None:
    ...
    tracks = Tracks()
    tr1 = tracks.add_track([Center(t1, 0, 0, 0)])
    tr2 = tracks.add_track([Center(t0, 10, 10, 0)])
    tr3 = tracks.add_track([Center(t0, 5, 5, 0)])
