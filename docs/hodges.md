...
**Reasoning**:
Original TRACK (`track_fail.c`) includes a mechanism to split trajectories if an exchange causes a point to exceed the maximum displacement ($d\_{max}$). This implementation checks displacement after each swap and breaks links that exceed the configured constraint.
...
$2n\_{overlap}/(n_1+n_2)$ and reports separation and intensity-difference
...
**Reasoning**: Storms move faster in the extratropics than the tropics. Applying a single $d\_{max}$ globally either misses fast mid-latitude storms or creates noise in the tropics.
