...
def main() -> None:
    ...
    track_durations = (grouped["time"].max() - grouped["time"].min()) / np.timedelta64(
        1, "h"
    )
    df["track_duration_hrs"] = df["track_id"].map(track_durations.to_dict())

    def calc_displacement(g: pd.DataFrame) -> float:
        return geod_dist_km(
            g.iloc[0]["lat"], g.iloc[0]["lon"], g.iloc[-1]["lat"], g.iloc[-1]["lon"]
        )
