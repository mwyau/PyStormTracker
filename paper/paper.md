...
* **Gather-then-Link Parallelism and Out-of-Core Processing:** In meteorological feature tracking, the *Detection* phase (identifying local extrema in 3D grids), as well as spherical harmonic filtering and Eulerian postprocessing (spatial binning), are embarrassingly parallel operations across the time dimension. PyStormTracker leverages Dask to parallelize these computationally intensive phases across workers using lazy evaluation. This map-reduce architecture enables out-of-core processing of massive datasets—such as multi-decadal, high-resolution CMIP6 runs exceeding 140 GB—without exceeding system memory. The system streams chunks from disk, extracts the sparse 1D feature arrays, and discards the meteorological grids. Conversely, the *Linking* phase is centralized: the main process gathers the reduced arrays from all workers and performs a single, sequential optimization pass. This orchestration guarantees perfect exactness and avoids the complex boundary-merging bugs common in spatial domain-splitting strategies.

* **Dynamic Calendar and Temporal Awareness:** Climate datasets utilize highly heterogeneous calendars, such as the `360_day` calendar in HadGEM models, `noleap` in CESM2, or the standard `gregorian` calendar in reanalyses like ERA5. Legacy tracking codes often rely on hardcoded season lengths (e.g., rigid 90-day assumptions), which introduces data drift and off-by-one errors during long-term integration. PyStormTracker natively utilizes `xarray` and `cftime` to read NetCDF `time:calendar` attributes, automatically and accurately handling temporal subsets, seasonal groupings, and unequal Gregorian months without requiring manual user intervention.
