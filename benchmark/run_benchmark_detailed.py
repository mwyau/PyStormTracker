...
def parse_output(output, version):
    ...
    if version == "v0.3.3":
        ...
        if comb_match:
            results["linking"] += float(comb_match.group(1)) # Merge combiner into linking
        ...
        det_match_dask = re.search(r"Task execution & gather time:\s*([0-9.]+)s", output)
        if det_match_dask:
...
def run_cmd(cmd, version):
    ...
    parsed["io_overhead"] = max(0.0, parsed["total"] - parsed["detection"] - parsed["linking"] - parsed["export"])
    
    print(f"  -> Wall: {wall_time:.2f}s | Det: {parsed['detection']:.2f}s | Link: {parsed['linking']:.2f}s | Exp: {parsed['export']:.2f}s | IO/Startup: {parsed['io_overhead']:.2f}s")
    return parsed
...
def main():
    ...
    for res_name, info in datasets.items():
        ...
        # Serial
        results[res_name]["serial"] = {"1": run_cmd(f"{base_cmd} {args} -b serial", version)}
        
        # Dask
        results[res_name]["dask"] = {}
        for w in [2, 4, 8]:
            results[res_name]["dask"][str(w)] = run_cmd(f"{base_cmd} {args} -b dask -w {w}", version)
            
        # MPI
        results[res_name]["mpi"] = {}
        for w in [2, 4, 8]:
            results[res_name]["mpi"][str(w)] = run_cmd(f"{mpi_cmd} -n {w} {mpi_target} {args} -b mpi", version)
